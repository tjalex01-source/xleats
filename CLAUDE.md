...
15. **The native customer app — STARTED**, lives in a **separate sibling repo `D:\Claude\xleats-customer`** (Expo/React Native, own git repo, no GitHub remote yet), sharing this Supabase backend. Built so far: email/password auth (role=customer), onboarding (birthday month/day + zip + device location + the two offer consents → the customer's own `profiles` row), best-effort Expo push-token registration → `devices`. Still to build there: follow trucks, "who's live / near me" discovery, truck detail, received offers/codes, notifications inbox. **Verification note:** it's a native app — can't be run/verified from the dev environment here; T.J. tests on-device via **Expo Go** (`npx expo start` → scan QR). Push *tokens* need a real dev build (Expo Go SDK 53+ can't fetch them), but the `/api/cron/push` delivery drain is already live and will deliver to any registered token.
